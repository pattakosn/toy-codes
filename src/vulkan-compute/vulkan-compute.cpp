...
int main() {
  try {
    ...
    vmaCreateBuffer(Allocator, &static_cast<VkBufferCreateInfo>(BufferCreateInfo), &AllocationInfo, &InBufferRaw, &InBufferAllocation, nullptr);

    AllocationInfo.usage = VMA_MEMORY_USAGE_GPU_TO_CPU;
    VmaAllocation OutBufferAllocation;
    vmaCreateBuffer(Allocator, &static_cast<VkBufferCreateInfo>(BufferCreateInfo), &AllocationInfo, &OutBufferRaw, &OutBufferAllocation, nullptr);
    ...
    auto AllocateBuffer = [Allocator, ComputeQueueFamilyIndex](size_t SizeInBytes, VmaMemoryUsage Usage) {
      ...
      BufferInfo Info;
      vmaCreateBuffer(Allocator, &static_cast<VkBufferCreateInfo>(BufferCreateInfo), &AllocationInfo, &Info.Buffer, &Info.Allocation, nullptr);

      return Info;
    };

    auto DestroyBuffer = [Allocator](BufferInfo Info) { vmaDestroyBuffer(Allocator, Info.Buffer, Info.Allocation); };
