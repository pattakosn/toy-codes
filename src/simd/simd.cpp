...
void multiply_vectors() {
  ...
  std::cout << std::endl;
  for (size_t i = 0; i < res.size(); ++i) std::cout << "res[ " << i << " ]= " << res[i] << std::endl;
  std::cout << std::endl;
}
...
void report_sse_support() {
  ...
  if ((d & (1 << 26)) != 0) std::cout << "SSE2 supported" << std::endl;
  if ((c & 1) != 0) std::cout << "SSE3 supported" << std::endl;
  if ((c & (1 << 19)) != 0) std::cout << "SSE4.1 supported" << std::endl;
  if ((c & (1 << 20)) != 0) std::cout << "SSE4.2 supported" << std::endl;
}
