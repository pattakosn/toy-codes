// http://www.selkie.macalester.edu/csinparallel/modules/CUDAArchitecture/build/html/1-Mandelbrot/Mandelbrot.html
#include <cstdio>
#include <cstdlib>
// #include <unistd.h>
// #include <err.h>
...
static void init_x11() {
  ...
  win = XCreateSimpleWindow(dpy, DefaultRootWindow(dpy), 0, 0, dim, dim, 0, black, white);
  ...
  bitmap = XCreateImage(dpy, visual, depth, ZPixmap, 0, (char *)malloc(dim * dim * 32), dim, dim, 32, 0);
...
void init_colours(void) {
  ...
  for (int i = 0; i < max_iter; i++) {
    char r = std::sin(freq * i + 3) * 127 + 128;
    char g = std::sin(freq * i + 5) * 127 + 128;
    char b = std::sin(freq * i + 1) * 127 + 128;
...
__global__ void mandel_kernel(uint32_t *counts, double xmin, double ymin, double step, int max_iter, int dim, uint32_t *colors) {
...
static void display_double(double xcen, double ycen, double scale, uint32_t *dev_counts, uint32_t *colors) {
...
#endif
#ifdef SHOW_X
  XPutImage(dpy, win, gc, bitmap, 0, 0, 0, 0, dim, dim);
  XFlush(dpy);
#endif
}
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    while (XPending(dpy) > 0) XNextEvent(dpy, &event);
    ...
    if ((event.type == ClientMessage) && ((Atom)event.xclient.data.l[0] == wmDeleteMessage)) break;
  }
